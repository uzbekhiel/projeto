<?
    ...
    class Virtudes extends BD{

        private $tabela = "virtudes";

        function pegaVirtudes (){
            $arrayAt = array();
        ...
        function listaDeVirtudes($num=3){
            $array = $this->pegaVirtudes();
            $lista = $array[0];
            for($i=1;$i<$num;$i++){
                $lista = $lista.",".$array[$i];
            }
    ...
    class virtudesFicha extends BD{

        function atribuiPonto($pontos,$jogador,$virtudes,$cronica) {
            $resultado1 = $this->pegaResultado($this->fazQuery("SELECT idantecedentes FROM atributos WHERE strNome = '".$virtudes."'"));
        ...
        function pegaPonto($jogador,$virtudes,$cronica){
            $resultado1 = $this->pegaResultado($this->fazQuery("SELECT idantecedentes FROM atributos WHERE strNome = '".$virtudes."'"));
            $resultado2 = $this->pegaResultado($this->fazQuery("SELECT idFicha FROM ficha WHERE strJogador = '".$jogador."'AND strCronica = '".$cronica."';"));
        ...
        function pontuaAntecedentes($jogador,$pontos,$cronica,$virtudes=null){
            if($virtudes == null){
                $virtudes = new Virtudes();
                $num = count($virtudes->pegaAtributos());
                $virtudes = $virtudes->listaDeantecedentes($num);
            }

            $arrayAt = explode(",", $virtudes);
        ...
        function retornaPontosAntecedentes($jogador,$cronica){
            $virtudes = new Virtudes();
            $num = count($virtudes->pegaAntecedentes());
            $arrayAt = explode(",", $virtudes->listaDevirtudes($num));
            $arrayPts = array();
